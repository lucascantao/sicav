...
<body>
    ...
    <div class="container">
        ...
        <div class="card-body">
            <h3 class="card-title text-center">Relatório de Visitas</h3>
            <hr>
            @if($visitas->isEmpty())
                <p class="card-text">Nenhuma visita encontrada com os critérios informados.</p>
            @else
                    ...
                    <tbody>
                        @foreach($visitas as $visita)
                            <tr>
                                ...
                                <td class="text-center">{{ $visita->pessoa->documento }}</td>
                                <td class="text-center">{{ $visita->pessoa->telefone1 }}</td>
                                <td class="text-center">{{ $visita->setor->sigla }}</td>
                                <td>{{ $visita->servidor->nome }}</td>
                                <td class="text-center">{{ $visita->created_at->format('d/m/y') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
